...
static inline void lawicelWriteChar(char c)
{
    serialBuffer.sendByteToBuffer((uint8_t)c);
}

static inline void lawicelWriteString(const char *s)
{
    serialBuffer.sendCharString((char *)s);
}

static inline void lawicelCR()
{
    serialBuffer.sendByteToBuffer('\r');
}
...
void LAWICELHandler::handleShortCmd(char cmd)
{
    ...
    case 'F':                      // LAWICEL - read status bits
        lawicelWriteString("F00"); // bit 0 = RX Fifo Full, 1 = TX Fifo Full, 2 = Error warning, 3 = Data overrun, 5= Error passive, 6 = Arb. Lost, 7 = Bus Error
        lawicelCR();
        break;
    case 'V': // LAWICEL - get version number
        lawicelWriteString("V1013");
        lawicelCR();
        SysSettings.lawicelMode = true;
        break;
    case 'N': // LAWICEL - get serial number
        lawicelWriteString("ESP32RET");
        lawicelCR();
        SysSettings.lawicelMode = true;
        break;
    case 'x':
        ...
        {
            lawicelWriteString("V2");
            lawicelCR();
        }
        else
        {
            lawicelWriteString("LAWICEL");
            lawicelCR();
        }
        break;
...
void LAWICELHandler::handleLongCmd(char *buffer)
{
    ...
    case 'Y': // SavvyCAN timestamp mode command
        // SavvyCAN commonly sends Y2
        // We can safely accept and ignore it
        break;
...
void LAWICELHandler::printBusName(int bus)
{
    ...
    case 0:
        // Serial.print("CAN0");
        serialBuffer.sendCharString((char *)"CAN0");
        break;
    case 1:
        // Serial.print("CAN1");
        serialBuffer.sendCharString((char *)"CAN1");
        break;
    default:
        // Serial.print("UNKNOWN");
        serialBuffer.sendCharString((char *)"UNKNOWN");
        break;
    }
}
...
void LAWICELHandler::sendFrameToBuffer(CAN_FRAME &frame, int whichBus)
{
    ...
    if (SysSettings.lawicellExtendedMode)
    {
        ...
        serialBuffer.sendCharString(buff);

        sprintf(buff, "%x", frame.id);
        serialBuffer.sendCharString(buff);

        if (frame.extended)
            serialBuffer.sendCharString((char *)" X ");
        else
            serialBuffer.sendCharString((char *)" S ");
        ...
        {
            sprintf(buff, " %x", frame.data.uint8[d]);
            serialBuffer.sendCharString(buff);
        }
    }
    else
    {
        if (frame.extended)
        {
            serialBuffer.sendByteToBuffer('T');

            sprintf(buff, "%08x", frame.id);
            serialBuffer.sendCharString(buff);
        }
        else
        {
